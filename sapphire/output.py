...
def write_checkpoint(states, dirpath, filename):
    ...
    for state in states:
        ...
        if time in stored_times:
            ...
            checkpointer.store(solution, name="solution")

            for key in state['post_processed_solutions'].keys():

                checkpointer.store(state['post_processed_solutions'][key], name="post_processed_"+key)
...
def report(sim, write_header=True):
    ...
    with sim.output_directory_path.joinpath("report").with_suffix(".csv").open("a+") as csv_file:
